...
import { AppStep, UserStats, MeasurementResult } from './types';
import { StatsForm } from './components/StatsForm';
import { CameraCapture } from './components/CameraCapture';
import { ResultsView } from './components/ResultsView';
import { analyzeBodyMeasurements } from './services/geminiService';
import { ScanLine, ArrowRight, Activity, Ruler } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleSideCapture = async (image: string) => {
    setSideImage(image);
    ...
    try {
      if (stats && frontImage) {
        // We use the just captured 'image' as the side image, 
        // and the state 'frontImage' for the front.
        const analysis = await analyzeBodyMeasurements(frontImage, image, stats);
        setResults(analysis);
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-4 md:p-8">
          ...
          <div className="max-w-4xl mx-auto text-center space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto mt-12 text-left">
              {[
                ...
                { title: 'Measure', desc: 'AI calculates skeletal structure and body volume.' }
              ].map((item, i) => (
            ...
            <button 
              ...
              className="mt-12 bg-blue-600 hover:bg-blue-500 text-white text-xl font-bold py-5 px-10 rounded-full shadow-xl hover:shadow-2xl transition-all transform hover:-translate-y-1 flex items-center gap-3 mx-auto"
            >
        ...
        {step === AppStep.Processing && (
          <div className="text-center animate-pulse">
             ...
             <p className="text-slate-500 mt-2">Processing Front and Side views for volumetric calculation.</p>
        ...
        {/* STEP 4: RESULTS */}
        {step === AppStep.Results && results && stats && frontImage && (
          <ResultsView results={results} stats={stats} onReset={handleReset} image={frontImage} />
        )}

      </main>

      <footer className="py-6 text-center text-slate-400 text-sm print:hidden">
        &copy; 2025 Fit Twin. Powered by Gemini 2.5 Flash.
      </footer>
    </div>
  );
};
